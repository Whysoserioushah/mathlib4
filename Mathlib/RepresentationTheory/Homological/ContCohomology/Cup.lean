...
* `ContinuousMap.continuous_prodMk`: the pairing `C(α, β) × C(α, δ) → C(α, β × δ)` is
  continuous whenever `α` is an R₁ space (in particular for every topological group), with no
  assumptions on `β` and `δ` and no local compactness assumption on `α`.
...
omit [ContinuousSMul k M1] [ContinuousSMul k M2] [TopologicalSpace G] [IsTopologicalGroup G] in
